...
impl EventEmitter for PancakeSwap {
						...
						loop {
							if let Ok(pool_resource_option) = aptos_client.get_account_resource(AccountAddress::from_str(resource_address).unwrap(), &pool.address).await {
								if let Some(pool_resource) = pool_resource_option.inner() {
									let amm: PancakeTokenPairMetadata = serde_json::from_value(pool_resource.data.clone()).unwrap();
									if value.is_none() {
										value = Some(amm);
										continue;
									}
									if value.as_ref().unwrap().ne(&amm) {
										pool.x_amount = amm.balance_x.value.0;
										pool.y_amount = amm.balance_y.value.0;
										value = Some(amm);
...
#[async_trait]
impl LiquidityProvider for PancakeSwap {
	...
	fn load_pools(&self) -> JoinHandle<()> {
		...
		tokio::spawn(async move {
			...
			if let Ok(resources) = wrapped_resources {
				for resource in resources.inner().into_iter() {
					match (
						...
						(&module, &resource_name) => {
							...
							let amm: PancakeTokenPairMetadata = serde_json::from_value(resource.data.clone()).unwrap();
				...
				eprintln!("{:?}: {:?}", LiquidityProviders::PancakeSwap, wrapped_resources.unwrap_err());
			}
			println!("{:?} Pools: {}",LiquidityProviders::PancakeSwap,  pools.read().await.len());
		})
	}
}
